...
let rec print_string_list = function
  | [] -> ()
  | head :: tail ->
    print_endline head.cards;
    print_string_list tail
;;

module Solver : Solver = struct
  let increment_value key array =
    if String.contains "23456789" key 
    then (array.((int_of_char key) - 49) <- array.((int_of_char key) - 49) + 1)
    else if String.contains "TJQKA" key then (
      let ind = 9 + (String.index "TJQKA" key) in
  ...
  let calculate_score cards =
    let evalute_hand status card_count =
      for i = 0 to Array.length card_count - 1 do status.(card_count.(i)) <- status.(card_count.(i)) + 1;
      done;
    ...
    let rec count_cards cards card_count   =
      match cards with
      | [] -> card_count
      | h :: t -> count_cards t (increment_value h card_count) in
    evalute_hand (Array.make 6 0) (count_cards cards (Array.make 14 0))
  let parse data =
    let to_hand line = 
      let splitted = (Str.split (Str.regexp " ") line) in
      {cards = (List.hd splitted); 
      bet = int_of_string (List.nth splitted 1); 
      score = (calculate_score (stringToList (List.hd splitted)))} in
    List.map to_hand (String.split_on_char '\n' data) 
  let compare_hands a b =
    if a.score < b.score then -1
    else if b.score < a.score  then 1
    else if compare_cards a.cards b.cards then 1
    else -1
  ...
  let task1 data =
    ...
    let sorted_hands = List.sort compare_hands all_hands in
  ...
  let increment_value key array =
    if String.contains "23456789" key 
    then (array.((int_of_char key) - 49) <- array.((int_of_char key) - 49) + 1)
    else if String.contains "TQKAJ" key then (
      let ind = 9 + (String.index "TQKAJ" key) in
      array.(ind) <- array.(ind) + 1);
    array
  ...
  let calculate_score cards =
    let evalute_hand status card_count =
      card_count.(find_max_index card_count) <- card_count.(find_max_index card_count) + card_count.(13);

      for i = 0 to Array.length card_count - 2 do status.(card_count.(i)) <- status.(card_count.(i)) + 1;
      done;
      if status.(5) = 1 then 6
      else if status.(4) = 1 then 5
      else if status.(3) = 1 && status.(2) = 1 then 4
